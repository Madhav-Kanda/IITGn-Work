...
class LinearRegression():
  ...
  def plot_line_fit(self, X, y, t_0, t_1):
        ...
        plt.xlabel("X")
        plt.ylabel("y")
        plt.title("theta0 = "+str(t_0)+ " theta1 = "+str(t_1))
  ...
  def plot_contour(self, X, y, t_0, t_1):
        ...
        # print(theta0,theta1)
        t0_min,t0_max,t1_min,t1_max = min(theta0),max(theta0), min(theta1),max(theta1)
        # print(t0_min,t0_max,t1_min,t1_max)
        a = np.arange(t0_min-0.5, t0_max+0.5, (t0_max - t0_min)/50)
        b = np.arange(t1_min-0.5, t1_max+0.5, (t1_max - t1_min)/50)
        ...
        y_predict = 0
        for j in range(len(X)):
            y_predict += (t_0 + (t_1)*(X.iloc[j][0]) - y[j])**2
        err=((np.sum(y_predict)/len(X))**(1/2))
        
        plt.figure()
        ax = plt.axes()
        ax.contour(t0, t1, error_mat)
        ax.scatter(t_0, t_1, color='b')
        plt.xlabel("thetha 0")
        plt.ylabel("thetha 1")
        plt.title("Error = "+str(err))
        #plt.savefig("plot_contour"+str(i))
        plt.show()
